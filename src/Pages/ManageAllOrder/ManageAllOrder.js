...
const ManageAllOrder = () => {
    ...
    useEffect(() => {
        fetch(`https://vast-castle-84903.herokuapp.com/buyer`)
            .then(res => res.json())
            .then(data => {
    ...
    const handleDelete = id => {
        ...
        if (proceed) {
            const url = `https://vast-castle-84903.herokuapp.com/buyer/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
    ...
    const handleApproved = (order) => {
        ...
        delete newOrder._id;
        const url = `https://vast-castle-84903.herokuapp.com/buyer/${order._id}`;
        fetch(url, {
            method: "PUT",
            headers: {
